...
// Helper function to extract a string from the ring buffer into a temporary
void UART1_GetRxBufferData(char *dst, uint32_t dst_size) {
  ...
  // turn off pause UART1 to avoid head/tail changes in copy
  UART1_IM_R &= ~0x50u;
  while ((tail != head) && (i < (dst_size - 1u))) {
...
bool UART1_WaitForPattern(const char *pattern, uint32_t timeout_ms)
{
    while (timeout_ms--)
    {
        if (UART1_RxBufferContainsPattern(pattern))
        {
            return true;
        }
